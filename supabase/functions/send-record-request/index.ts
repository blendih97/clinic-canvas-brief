...
Deno.serve(async (req) => {
  ...
  // Requesting records is a paid action — verify server-side, not just in the UI.
  ...
  const { data: paid } = await admin.rpc("user_has_paid_access", { _user_id: requesterId });
  if (!paid) {
    return new Response(JSON.stringify({ error: "Upgrade required", code: "upgrade_required" }), {
...
<body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="border-bottom: 2px solid #b8860b; padding-bottom: 15px; margin-bottom: 25px;">
    <h1 style="font-family: Georgia, serif; font-size: 24px; font-weight: 300; letter-spacing: 0.15em; color: #b8860b; margin: 0;">RinVita</h1>
    <p style="font-size: 10px; letter-spacing: 0.15em; color: #888; margin: 2px 0 0;">Your health history. Everywhere you go.</p>
  </div>

  <p>Dear ${providerName || "Healthcare Provider"} team,</p>

  <p>Your patient <strong>${patientName}</strong> has requested access to their medical records through RinVita, a secure health record platform.</p>
  ...
  <p>Please use the secure link below to upload the requested documents directly to their health record. No login is required — simply click the link and upload the files.</p>
  ...
  <p style="font-size: 13px; color: #666;">If you have any questions please contact <a href="mailto:info@rinvita.co.uk" style="color: #b8860b;">info@rinvita.co.uk</a></p>
  ...
  <div style="border-top: 1px solid #eee; margin-top: 30px; padding-top: 15px;">
    ...
    <p style="font-size: 11px; color: #999; margin: 2px 0 0;">Your health history. Everywhere you go.</p>
  </div>
</body>
</html>`;
