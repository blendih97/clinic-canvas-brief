  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await supabase.from("documents").insert({
        user_id: request.user_id, name: file.name, type: "Received Records",
        date: new Date().toISOString().split("T")[0], facility: request.provider_name,
        country: "", pages: 1, extracted: false, file_url: filePath,
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="bg-card border border-border rounded-xl p-8 max-w-md text-center">
          ...
          <p className="text-sm text-muted-foreground">
            The records have been securely uploaded to {firstName}'s health record. You can close this page.
          </p>
        </div>
      </div>
    );
          ...
          >
            ...
            {file ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground mt-1">PDF, JPEG, PNG, DICOM, MP4, Word documents accepted</p>
              </>
            )}
          </div>
