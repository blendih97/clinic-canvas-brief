      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="bg-card border border-border rounded-xl p-8 max-w-md text-center">
          ...
          <h2 className="font-heading text-xl text-foreground mb-2">
            {notFound ? "Link Not Found" : "Link Expired"}
          </h2>
          <p className="text-sm text-muted-foreground">
            {notFound ? "This share link is invalid." : "This share link has expired."}
          </p>
        </div>
      </div>
    );
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-foreground">{new Date(share.created_at).toLocaleDateString()}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Expires</span>
              <span className="text-foreground">{new Date(share.expires_at).toLocaleDateString()}</span>
            </div>
          </div>
          ...
          <p className="text-[10px] text-muted-foreground text-center">
            This file is shared via Vault Health Intelligence. Original file integrity is preserved.
          </p>
        </div>
      </div>
    </div>
