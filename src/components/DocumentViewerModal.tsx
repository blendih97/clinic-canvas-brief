...
import { X, FileText, Download, Share2, Globe, Languages, Loader2, AlertCircle } from "lucide-react";
...
const DocumentViewerModal = ({ document: doc, onClose, onShare }: Props) => {
  ...
  // Use filePath first, fall back to fileUrl for legacy
  ...
  const hasOriginalText = summaryContent.originalText.length > 0;
  const hasOriginalImaging = relatedImaging.some(i => (i as any).findingOriginal);
      ...
      <div className="bg-background rounded-xl border border-border w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-auto flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 p-5 overflow-auto space-y-5">
            {/* Language toggle */}
            {originalLang && originalLang !== "English" && (
              <div className="flex gap-1 bg-muted rounded-lg p-1">
                ...
                >
                  <Languages className="w-3 h-3" /> {originalLang}
                </button>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              {originalLang && originalLang !== "English" && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-foreground">{originalLang}</span>
                </div>
              )}
            </div>
              ...
              <div>
                ...
                {hasOriginalText ? (
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    Original-language text was not saved for this record. Re-upload the document to extract both English and original text.
                  </p>
                )}

                {/* Imaging findings in original language */}
                {relatedImaging.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-xs tracking-wider text-muted-foreground uppercase font-medium mb-2">Imaging Findings ({originalLang})</h4>
                    <div className="space-y-2">
